        ...
        <div class="container px-5 py-12 mx-auto">
            <div class="mb-12 flex items-center">
                ...
                <form action="{{ route('logout') }}" method="post">
                    @csrf
                    <button type="submit" class="ml-3 text-orange-500 border border-orange-500 rounded py-0.5 px-1.5 hover:bg-orange-500 hover:text-white">Sign Out</button>
                </form>
            </div>
            <div class="-my-6">
                    ...
                    >
                        ...
                        <div class="md:flex-grow mr-8 mt-2 flex items-center justify-start">
                            @foreach($listing->tags as $tag)
                                <span class="inline-block mr-2 tracking-wide text-orange-500 text-xs font-medium title-font py-0.5 px-1.5 border border-orange-500">{{ strtoupper($tag->name) }}</span>
                            @endforeach
                        </div>
                        <span class="md:flex-grow flex flex-col items-end justify-center">
                            <span>{{ $listing->created_at->diffForHumans() }}</span>
                            <span><strong class="text-bold">{{ $listing->clicks()->count() }}</strong> Apply Clicks</span>
                        </span>
                    </a>
                @endforeach
            </div>
